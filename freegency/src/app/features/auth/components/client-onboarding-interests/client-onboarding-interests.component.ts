...
@Component({
  selector: 'app-client-onboarding-interests',
  imports: [HugeiconsIconComponent],
  templateUrl: './client-onboarding-interests.component.html',
  styleUrl: './client-onboarding-interests.component.css',
})
export class ClientOnboardingInterestsComponent implements OnInit {
  ...
  private load(): void {
    ...
    forkJoin({
      ...
      interests: this.profileApi.getClientInterests().pipe(catchError(() => of([]))),
    }).subscribe({
  ...
  private async submit(saveInterests: boolean): Promise<void> {
    ...
    try {
      if (saveInterests) {
        await firstValueFrom(this.profileApi.replaceClientInterests(this.selectedIds()));
      }
      await this.router.navigate([`${CLIENT_ONBOARDING_PATH}/create-project`]);
    } catch (err) {
